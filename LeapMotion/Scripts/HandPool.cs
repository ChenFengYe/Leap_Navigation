...
namespace Leap.Unity {
  ...
  public class HandPool :
    ...
    //SteamVR_ControllerManager VR_ControllerManager = GameObject.Find("VR Interface(L_Teleporting & R_Operating)").GetComponent<SteamVR_ControllerManager>();
    ...
    public void ReturnToPool(IHandModel model) {
      ...
      for (int i = 0; i < activeHandReps.Count; i++) {
        ...
        if (rep.RepChirality == model.Handedness && rep.RepType == model.HandModelType) {
          ...
          if (!modelFromGroupFound && model.isRemote == rep.IsRemote_)
          {
            rep.AddModel(model);
