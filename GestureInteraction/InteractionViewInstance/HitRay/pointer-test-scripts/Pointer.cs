...
public class Pointer : MonoBehaviour
{
	...
	public PointerType pointerType = PointerType.Line;
	...
	public bool _teleportActive = true;
	...
	void Start()
	{
		...
		if (teleportActive) EnableTeleport();
	}
	...
	private void CalculateArc()
	{
		...
		while (i < 500)
		{
			if (pointerType == PointerType.Line)
			{
				float to_length = (maxDistance * 0.01f) * _vrPlayArea.localScale.magnitude;
				Ray m_ray = new Ray(currentPosition, currentDirection);
				...
				{
					float maxDist = 50.0f;
					positions1.Add(maxDist * Vector3.forward);
					totalDistance1 += maxDist;
				}
				break;
			}
	...
	virtual public IEnumerator Teleport()
	{
		if (teleportActive && _goodSpot)
		{
			...
			Debug.Log("teleport");
			Vector3 transition = _teleportSpot + offset - _vrPlayArea.position;
			for (int ii = 0; ii < 10; ii++)
			{

				_vrPlayArea.position += transition / 10;
				//_vrPlayArea.Translate((_teleportSpot + offset - _vrPlayArea.position) / 10, Space.World);
				Debug.Log(_vrPlayArea.position);
